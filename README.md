...
> **[In-Context Forecasting](./scripts/in_context_forecasting/)**: We propose in-context forecasting for the first time, where time series prompts can be incorporated into the input context to enhance forecasting.
...
2. Download the large language models from [Hugging Face](https://huggingface.co/). The default LLM is MiniCPM5, and the default checkpoint directory is `./MiniCPM5`. You can change the `llm_ckp_dir` in `run.py` to use other LLMs.
   ...
   * [GPT2](https://huggingface.co/openai-community/gpt2)
   ...
   For example, if you download and put the MiniCPM5 directory successfully, the directory structure is as follows:
   - data_provider
   - dataset
   - MiniCPM5
     - config.json
