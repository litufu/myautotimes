...
def handle_single_df(df):
    ...
    df_copy = df[['datetime', 'open', 'close', 'high', 'low', 'volume', 'amount']].copy()
    return df_copy
...
def get_stock_data(stock_code):
    folders = [f for f in os.listdir(r'D:\BaiduNetdiskDownload\stock\minute15') if os.path.isdir(os.path.join(r'D:\BaiduNetdiskDownload\stock\minute15', f))]
    df_list = []
    for folder in folders:
        file_path = os.path.join(r'D:\BaiduNetdiskDownload\stock\minute15', folder, stock_code + '.csv')
